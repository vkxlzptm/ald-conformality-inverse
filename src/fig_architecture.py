...
prof = [1 - 0.92 * zc ** 1.5, 1 - 0.80 * zc ** 1.6, 1 - 0.62 * zc ** 1.8]
cols = ["#7fb3d5", "#2874a6", "#154360"]
labs = ["200 °C", "250 °C", "300 °C"]
ax_in = axL.inset_axes([0.025, 0.775, 0.175, 0.195])
for p_, c, l in zip(prof, cols, labs):
    ax_in.plot(p_, zc, lw=2.0, color=c, label=l)
rng = np.random.default_rng(3)
miss = rng.choice(64, 10, replace=False)
ax_in.plot(prof[1][miss], zc[miss], "x", ms=5, color="#c0392b")
...
axL.text(0.112, 0.700, "3 temperatures  ×  64 depth bins\n"
...
axL.text(0.60, 0.740,
         "7 input channels  =  3 coverage  +  3 log coverage  +  1 mask.  The log\n"
         "channels keep small values deep in the feature alive; the mask channel\n"
         "separates 'not measured' from 'bare'.  Convolution runs over depth.",
         ha="center", va="top", fontsize=8.8, color=GREY)

# --------------------------------------------------------------- bottom row
YB, HB = 0.40, 0.155
box(axL, 0.035, YB, 0.185, HB, "#fdf2e2", ORANGE, "Known conditions",
    "log AR,   log dose", 10.5, 9)
box(axL, 0.285, YB, 0.175, HB, "#fdf2e2", ORANGE, "Global pooling",
    "mean ⊕ max  →  384", 10.5, 9)
box(axL, 0.525, YB, 0.175, HB, "#f4ecf7", "#7d3c98", "Dense layers",
    "386 → 384 → 384,  GELU", 10.5, 9)
box(axL, 0.765, YB, 0.205, HB, "#fdedec", "#c0392b", "Mixture-density head",
    "8 components × 5 parameters", 10.5, 8.8)
...
axL.text(0.1275, 0.375, "absolute dose, never $\\Pi_2$ —\n"
         "its denominator is the unknown site density",
         ha="center", va="top", fontsize=8.4, color="#a04000")

axL.text(0.5, 0.245, "657,976 parameters   ·   35 s per epoch on 6 CPU cores"
         "   ·   one forward pass is milliseconds",
         ha="center", fontsize=11, color=INK)
axL.text(0.5, 0.175,
         "The head predicts $(s_{0,\\mathrm{ref}},\\ E_a)$, not one sticking "
         "probability per temperature:\n"
         "$s(T)$ is then the analytic Arrhenius form, so temperature "
         "extrapolation is structural rather than learned.",
...
ax_p.set_ylabel("log site density   (normalized)", fontsize=9.5)
...
axR.text(0.5, 0.185,
         ...
         "probability and the site density together leaves the profile\n"
